%--------------------------------------------------------------------------
% Name:            knnclassify_modified.m
%
% Description:     A modified version of the knnclassify.m m-code. 
%                  The only difference is in the outputs of the function.
%
% Outputs:         outClass - final label assigned to news articles
%                  LL - First top two labels
%                  SDIS - Number of articles (score) of the first top two labels
%
...
function [outClass,LL, SDIS] = knnclassify_modified(sample, TRAIN, group, K, distance,rule)
...
%         if ng==0
%             ng=ng+1
%             OC(ng,1) = classes(ii,1);
%         elseif(OC(ng,1)~=classes(ii,1))
%            ng=ng+1
%             OC(ng,1) = classes(ii,1); 
%         end
%     ii=ii+1;
%     end 
    ...
    for ju = 1:length(outClass(:,1))
       
        yu = find(outClass(ju,1)==clas(:,1));
        SDIS(ju,1) = dSd(yu(1,1),1);
